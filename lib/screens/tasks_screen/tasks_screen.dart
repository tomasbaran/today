...
class _TasksScreenState extends State<TasksScreen> {
  ...
  void initState() {
    ...
    if (widget.date != null) {
      screenManager.getList(date: widget.date);
    }
    if (widget.listId != null) {
      screenManager.getList(listId: widget.listId);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => TaskService().addTask(date: widget.date),
        label: Text('+ Add task'),
      ),
      appBar: AppBar(
        ...
        title: Column(
          children: [
            ValueListenableBuilder(
                valueListenable: screenManager.listNotifier, builder: ((context, selectedList, child) => Text(selectedList.title ?? '...'))),
          ],
        ),
        actions: <Widget>[
          IconButton(
          ...
          IconButton(
            ...
            onPressed: () {
              ...
              screenManager.getList(date: now.add(Duration(days: 1)));
            },
          ),
        ],
      ),
